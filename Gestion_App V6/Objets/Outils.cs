...
namespace Gestion
{
    public static class Outils
    {
        ...
        public static int BoolToInt(this Boolean Bl)
        {
            return Bl ? 1 : 0;
        }

        public static String ToAsci(this String text)
        {
        ...
        public static String PrepareForFilename(this String text)
        {
            String ListeChar = "\\/:*?\"<>|";
        ...
        public static string GetEnumUnite(this Enum value)
        {
            FieldInfo fi = value.GetType().GetField(value.ToString());

            UniteAttribute[] attributes = (UniteAttribute[])fi.GetCustomAttributes(typeof(UniteAttribute), false);

            if (attributes != null && attributes.Length > 0)
                return attributes[0].Unite;
            else
                return "";
        }

        public static void Concatener(this MySqlParameterCollection CollBase, MySqlParameterCollection CollConcat)
        {
            foreach (MySqlParameter Item in CollConcat)
                CollBase.Add(Item);
        }
